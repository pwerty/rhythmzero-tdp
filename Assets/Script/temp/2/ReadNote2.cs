...
public class ReadNote2 : MonoBehaviour
{
        ...
        foreach (string line in lineData)
        {
            if (line.StartsWith("#"))
            {
                ...
                else if (data[0].IndexOf(":") != -1)
                {
                    ...
                    int bar = 0;
                    Int32.TryParse(data[0].Trim().Substring(1, 3), out bar); //첫번째 글자에서 부터 3글자 load 

                    int channel = 0;
                    Int32.TryParse(data[0].Trim().Substring(4, 2), out channel); //네번째 글자에서 부터 2글자 load

                    string noteStr = data[0].Trim().Substring(7); //일곱 번째 글자에서 부터 load
    ...
    { //위에서 가져 온 데이터들 이용
        ...
        if (tempStr.Length != 0)
        {
            totalBeatOfBar = tempStr.Length / 2; // '00', '01' 등 2개가 하나의 노트이므로 /2를 한다. //64개 데이터에서 나누기 2 = 32노트
        }
